...
def load_point_pillar_params_shrink(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
    ...
    param : dict
        Modified parameter dictionary with new attribute.
    """
    cav_lidar_range = param['point_pillar_shrink']['preprocess']['cav_lidar_range']
    voxel_size = param['point_pillar_shrink']['preprocess']['args']['voxel_size']

    grid_size = (np.array(cav_lidar_range[3:6]) - np.array(
        cav_lidar_range[0:3])) / \
                np.array(voxel_size)
    grid_size = np.round(grid_size).astype(np.int64)
    param['model']['args']['point_pillar_shrink']['point_pillar_scatter']['grid_size'] = grid_size

    anchor_args = param['point_pillar_shrink']['postprocess']['anchor_args']
    ...
    param['point_pillar_shrink']['postprocess'].update({'anchor_args': anchor_args})
...
def load_point_pillar_params_ori(param):
    """
    Based on the lidar range and resolution of voxel, calcuate the anchor box
    and target resolution.
